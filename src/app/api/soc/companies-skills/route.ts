...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!jwtToken) {
      console.error("[SOC Companies Skills API] JWT_TOKEN_SECRET not configured");
      return NextResponse.json(
    ...
    const externalUrl = `https://careerexplorer.hawaii.edu/api/kamaaina_pathways/soc5_to_companies_skills.php?t=${jwtToken}&soc5=${soc5}`;
    console.log(`[SOC Companies Skills API] Calling URL: ${externalUrl.replace(jwtToken, '[REDACTED]')}`);
    ...
    if (trimmedResponse.startsWith('{') || trimmedResponse.startsWith('[')) {
      ...
      console.log(`[SOC Companies Skills API] ⚠️  Received PHP array, converting to JSON...`);
      try {
        ...
        console.error(`[SOC Companies Skills API] Failed to convert PHP output:`, conversionError.message);
        return NextResponse.json(
          { 
      ...
      console.error(`[SOC Companies Skills API] Response is not JSON:`, responseText.substring(0, 500));
      return NextResponse.json(
        { 
