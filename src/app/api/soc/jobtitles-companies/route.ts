...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!jwtToken) {
      console.error("[SOC Job Titles Companies API] JWT_TOKEN_SECRET not configured");
      return NextResponse.json(
    ...
    const externalUrl = `https://careerexplorer.hawaii.edu/api/kamaaina_pathways/soc5_to_jobtitles_companies.php?t=${jwtToken}&soc5=${soc5}`;
    console.log(`[SOC Job Titles Companies API] Calling URL: ${externalUrl.replace(jwtToken, '[REDACTED]')}`);
    ...
    if (trimmedResponse.startsWith('{') || trimmedResponse.startsWith('[')) {
      ...
      console.log(`[SOC Job Titles Companies API] ⚠️  Received PHP array, converting to JSON...`);
      try {
        ...
        console.error(`[SOC Job Titles Companies API] Failed to convert PHP output:`, conversionError.message);
        return NextResponse.json(
          { 
      ...
      console.error(`[SOC Job Titles Companies API] Response is not JSON:`, responseText.substring(0, 500));
      return NextResponse.json(
        { 
